    ...
    for (let i = 1; i <= 75; i++) {
      checkboxes.push(
        <li key={i} className="relative flex items-center">
          <label
            htmlFor={`checkbox-${i}`}
            className={`flex items-center justify-center border border-mybl rounded-md cursor-pointer px-1 md:p-3 
                ${
                  selectedStudents.includes(i.toString())
                    ? "bg-mybl  bg-opacity-75"
                    : "hover:bg-gray-200"
                } ${selectedTimeSlot ? "" : "bg-slate-100 hover:bg-slate-100 cursor-not-allowed text-black"}`}
          >
            <input
              id={`checkbox-${i}`}
              type="checkbox"
