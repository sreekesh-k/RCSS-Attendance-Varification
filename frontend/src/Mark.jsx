...
function Mark() {
  ...
  useEffect(() => {
    if (selectedCourse.id) {
      fetch(
        `${import.meta.env.REACT_APP_API_BASE_URL}/college/timetables?cid=${
          selectedCourse.id
        }&day=${day}`
      )
  ...
  return (
    <>
      <main className="relative w-full min-h-[88svh] grid place-items-center overflow-hidden mt-3">
        <div className=" grid w-11/12 place-items-center grid-rows-2 md:grid-cols-2 md:grid-rows-1 text-black p-10 border-2 border-mybl rounded-xl bg-slate-50">
          <div className=" w-full h-full flex flex-col items-center justify-between border-b-2 md:border-b-0 border-b-mybl md:border-r-2 md:border-r-mybl py-10 md:p-5">
            <DateSelector
          ...
          <div className="w-full h-full flex flex-col  py-5 md:p-5">
            <AbsentCheckboxList
